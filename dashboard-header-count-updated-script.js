...
async function prepairEnablementByProductData(datas, productHashMap, processHashMap) {
    for (const data of datas) {
        let date = getYYMMDDdate(JSON.stringify(data.createdAt));
        let month = getMonth(date);
        month = month - 1;
        ...
        let arrayIndex = `${data.tenantId}-${data.processId}-${data.productUPC}-${data.siteId}-${date}`
...
mongoDb.once('open', async function () {
    ...
    let tenantIdArray = ['c2107b22-b02e-45a7-b126-89b65b054ef6', '8dd5c1c6-def0-42b5-a450-3630d763fe92']
    for (const tenantId of tenantIdArray) {
        ...
        while (true) {
            ...
            await prepairEnablementByProductData(tenantData, productHashMap, processHashMap);
